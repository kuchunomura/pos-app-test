...
const SS_ID = '1_iNytjWzVi6vvd_nSecG_uZJEnzkoL8zynY12UkNrNM'; // デフォルト（フォールバック用）
...
function promptSwitchSpreadsheet() {
  ...
  var html = '<style>'
    ...
    + 'google.script.run'
    + '.withSuccessHandler(function(){document.getElementById("res").textContent="✅ 切り替えました";setTimeout(google.script.host.close,1500);})'
...
function createInvoiceSheet(data) {
  ...
  var hRow = 6;
  out.getRange(hRow,1,1,4).setValues([['品名','数量','単価','金額']]);
  ...
  totalRow++;
  out.getRange(totalRow,1,1,6).merge().setValue('　').setBackground(MAIN);
  totalRow++;
  ...
  out.getRange(1,1,totalRow,4).setBorder(true,true,true,true,true,true,BORDER,SpreadsheetApp.BorderStyle.SOLID);
